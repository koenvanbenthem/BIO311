...
<<ploti,echo=c(-1),out.width='0.5\\textwidth',fig.align='center'>>=
par(mar=c(6,6,2,2))
rot$Total<-rot$Alive_Juv+rot$Alive_Adult
rot_copp<-aggregate(rot$Total,by=list(rot$Copper),mean)
plot(rot_copp$Group.1,rot_copp$x,
     xlab="Copper treatment",ylab="Avg population size")
@
...
<<ploti2,echo=c(-1),out.width='0.5\\textwidth',fig.align='center'>>=
par(mar=c(6,6,2,2))
rot_copp$Group.1<-factor(rot_copp$Group.1,
                         levels=c("low","medium","high"),
                         ordered=TRUE)

plot(rot_copp$Group.1,rot_copp$x,main="Total Population",
     xlab="Copper treatment",ylab="Avg population size")
...
<<ploti3,echo=F,out.width='0.75\\textwidth',fig.align='center'>>=
par(mar=c(6,6,2,2),mfrow=c(2,1))

rot_copp<-aggregate(rot$Alive_Juv,by=list(rot$Copper),mean)
rot_copp$Group.1<-factor(rot_copp$Group.1,
                         levels=c("low","medium","high"),
                         ordered=TRUE)
plot(rot_copp$Group.1,rot_copp$x,main="Juveniles",
     xlab="Copper treatment",ylab="Avg number of juveniles")
rot_copp<-aggregate(rot$Alive_Adult,by=list(rot$Copper),mean)
rot_copp$Group.1<-factor(rot_copp$Group.1,
                         levels=c("low","medium","high"),
                         ordered=TRUE)
plot(rot_copp$Group.1,rot_copp$x,main="Adults",
     xlab="Copper treatment",ylab="Avg number of adults")
...
temp2<-subset(Tot_mean,Group.1=="Pollution")
lines(temp2$Group.2,temp2$x,col='orange',pch=1,type="b")
legend(x=1, y=24, col=c("red","blue","green","orange"),legend=c("Commercial","Postpollution","Recovery","Pollution"), lty=1, pch=1, bty="n")
temp2<-subset(Tot_mean,Group.1=="Commercial")
...
<<gekkeplot,echo=FALSE,out.width='0.65\\textwidth',fig.align='center',warning=FALSE>>=
RData<-rot
