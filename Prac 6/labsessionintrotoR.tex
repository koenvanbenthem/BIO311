...
\begin{verbatim}
##          Group.1 Group.2     x
## 1     Commercial    high 5.889
## 2      Pollution    high 2.056
## 3  Postpollution    high 1.722
## 4       Recovery    high 2.611
## 5     Commercial     low 7.111
## 6      Pollution     low 2.667
## 7  Postpollution     low 1.667
## 8       Recovery     low 2.944
## 9     Commercial  medium 5.833
## 10     Pollution  medium 3.833
## 11 Postpollution  medium 2.444
## 12      Recovery  medium 1.889
\end{verbatim}
\end{kframe}
\end{knitrout}
...
Let us have a look at how your rotifer population does over time. For this we use the plot function in R. The first argument is the $x$-axis and the second argument is the $y$-axis (i.e. your variable). At this stage, you may spot some outliers, in which case it can be a good idea to look back at the values you entered in the cell as you may have made a mistake.

We will now plot the total number of living individuals (both juveniles and adults) against the copper concentration. The \texttt{xlab} and \texttt{ylab} options are used for the labels of the axes. The \texttt{main} option is used to set the title of the graph.
\begin{knitrout}
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlstd{rot}\hlopt{$}\hlstd{Total}\hlkwb{<-}\hlstd{rot}\hlopt{$}\hlstd{Alive_Juv}\hlopt{+}\hlstd{rot}\hlopt{$}\hlstd{Alive_Adult}
\hlstd{rot_copp}\hlkwb{<-}\hlkwd{aggregate}\hlstd{(rot}\hlopt{$}\hlstd{Total,}\hlkwc{by}\hlstd{=}\hlkwd{list}\hlstd{(rot}\hlopt{$}\hlstd{Copper),mean)}
\hlkwd{plot}\hlstd{(rot_copp}\hlopt{$}\hlstd{Group.1,rot_copp}\hlopt{$}\hlstd{x,}
     \hlkwc{xlab}\hlstd{=}\hlstr{"Copper treatment"}\hlstd{,}\hlkwc{ylab}\hlstd{=}\hlstr{"Avg population size"}\hlstd{)}
\end{alltt}
\end{kframe}

{\centering \includegraphics[width=0.5\textwidth]{figure/ploti} 
...
We see now that the order of the treatments is a bit annoying: the level high comes before the level low. To change this, we need to change the column \texttt{Group.1} in \texttt{rot\textunderscore copp} to the type \texttt{factor}:
\begin{knitrout}
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlstd{rot_copp}\hlopt{$}\hlstd{Group.1}\hlkwb{<-}\hlkwd{factor}\hlstd{(rot_copp}\hlopt{$}\hlstd{Group.1,}
                         \hlkwc{levels}\hlstd{=}\hlkwd{c}\hlstd{(}\hlstr{"low"}\hlstd{,}\hlstr{"medium"}\hlstd{,}\hlstr{"high"}\hlstd{),}
                         \hlkwc{ordered}\hlstd{=}\hlnum{TRUE}\hlstd{)}

\hlkwd{plot}\hlstd{(rot_copp}\hlopt{$}\hlstd{Group.1,rot_copp}\hlopt{$}\hlstd{x,}\hlkwc{main}\hlstd{=}\hlstr{"Total Population"}\hlstd{,}
     \hlkwc{xlab}\hlstd{=}\hlstr{"Copper treatment"}\hlstd{,}\hlkwc{ylab}\hlstd{=}\hlstr{"Avg population size"}\hlstd{)}
\end{alltt}
\end{kframe}

{\centering \includegraphics[width=0.5\textwidth]{figure/ploti2} 
...
{\centering \includegraphics[width=0.5\textwidth]{figure/gp5} 
...
{\centering \includegraphics[width=0.5\textwidth]{figure/gp6} 
...
\end{alltt}
\end{kframe}
\includegraphics[width=\maxwidth]{figure/gp10} 
...
{\centering \includegraphics[width=0.6\textwidth]{figure/blabla} 
