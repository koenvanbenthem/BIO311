...
M1<-matrix(c(0.73,0.27,0.50,0.50),2,2,byrow=T) 
M2<-matrix(c(0.5,0.5,0.2,.8),2,2,byrow=T)
@
...
Migration
<<MPD2,echo=TRUE,out.width='0.5\\textwidth',fig.align='center'>>=
cat("\\begin{align*}")
cat("\\boldsymbol{B_1} &=")
 print(xtable(B1,align=rep("",ncol(B1)+1)), floating=FALSE, tabular.environment="pmatrix", 
 hline.after=NULL, include.rownames=FALSE, include.colnames=FALSE)
...
Simulation approach
<<MPD3,echo=TRUE,out.width='0.5\\textwidth',fig.align='center'>>=
@
