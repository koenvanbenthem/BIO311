...
<<MPD1,echo=TRUE,out.width='0.5\\textwidth',fig.align='center'>>=
# Metapopulation Model (Hunter & Caswell, 2005 - Ecological Modelling)

rm(list=ls(all=TRUE))
library(popbio)


# Peregrine example (Wootton & Bell 1992)


# Enter demography B{1,2,..,s} and movement M{1,2,..,p} matrices

# Demography

B1<-matrix(c(0,0.26,0.72,0.77),2,2,byrow=T)    # for patch 1
B1 #check
eigen.analysis(B1) # basic asymptotic analysis including population growth rate (lambda) and sensitivity matrix

B2<-matrix(c(0,0.19,0.72,0.77),2,2,byrow=T)	   # for patch 2
B2  #check
eigen.analysis(B2) # basic asymptotic analysis


# Movement

M1<-matrix(c(0.73,0.27,0.50,0.50),2,2,byrow=T)  # for juveniles
M1 #check

M2<-matrix(c(0.5,0.5,0.2,.8),2,2,byrow=T) # for adults
M2 #check
