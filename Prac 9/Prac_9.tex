...
{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in t(A): object 'A' not found}}\begin{alltt}
\hlstd{eigens.At}\hlkwb{<-}\hlkwd{eigen}\hlstd{(At)}
\end{alltt}


{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in as.matrix(x): object 'At' not found}}\begin{alltt}
\hlstd{eigens.At}
\end{alltt}


{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in eval(expr, envir, enclos): object 'eigens.At' not found}}\end{kframe}
\end{knitrout}
...
{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in which.max(eigens.At\$values): object 'eigens.At' not found}}

{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in eval(expr, envir, enclos): object 'eigens.At' not found}}

{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in eval(expr, envir, enclos): object 'eigens.At' not found}}

{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in eval(expr, envir, enclos): object 'v' not found}}

{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in eval(expr, envir, enclos): object 'rv' not found}}\end{kframe}
\end{knitrout}
\begin{mdframed}
...
\begin{verbatim}
##          [,1]      [,2]       [,3]
## [1,] 2.211456 0.2794879 1.21900874
## [2,] 2.207566 3.2328849 0.01089951
## [3,] 1.810142 3.6920773 3.47931309
\end{verbatim}
\begin{alltt}
...
\end{alltt}
\begin{verbatim}
## [1] 0.3802276+0i 0.3967160+0i 0.2230563+0i
\end{verbatim}
\begin{alltt}
...
\end{alltt}
\begin{verbatim}
## [1] 0.3802276 0.3967160 0.2230563
\end{verbatim}
\begin{alltt}
...
\end{kframe}
\end{knitrout}
You can interpret this as follows: if we for example start with a population $\begin{pmatrix}1\\ 1\\ 1 \end{pmatrix}$, after $40$ years, $38\% $ of the total population will have originated from the individual that was initially in age class $1$, $40\% $ from the individual in age class $2$ and the remaining $22\% $ of the total population at timestep $40$ from the individual that was in age class $3$ in the initial population.
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{verbatim}
## [1] 0.5509637 0.2874280 0.1616084
\end{verbatim}
\end{kframe}
\end{knitrout}
This means that after $40$ years, $55\% $ of the total population will have originated from the two individuals that were initially in age class $1$, $29\% $ from the individual in age class $2$ and the remaining $16\% $ of the total population at timestep $40$ from the individual that was in age class $3$ in the initial population. These numbers do not correspond to the left eigenvector, yet they do relate to the left eigenvalue.
First we need to realize that if $55\% $ of the final population originated from the two individuals in age class 1 in the initial population, this means that each of these two individuals is responsible for $27.5\% $ of the final population. It is of course fairer to compare these numbers with each other (so after correcting for the number of individuals that we started with):
\begin{equation*}
\begin{pmatrix}
27.5 \\
29 \\
...
{\ttfamily\noindent\itshape\color{messagecolor}{\#\# Loading required package: quadprog}}\end{kframe}
\end{knitrout}
\begin{equation}
...
Before we can start with the analysis of the population matrices, we first need... the matrices and thus the rates (survival and reproduction). Here, we have only two stages: juveniles and adults, so we consider a $2\times 2$ matrix. 
\subsection{LTRE}
\begin{enumerate}
...
\hlstd{get_matrix}\hlkwb{<-}\hlkwa{function}\hlstd{(}\hlkwc{roti}\hlstd{,}\hlkwc{pop}\hlstd{,}\hlkwc{cop}\hlstd{)\{}
  \hlkwa{if}\hlstd{(}\hlopt{!}\hlstd{pop} \hlopt{%in%} \hlkwd{c}\hlstd{(}\hlkwd{levels}\hlstd{(roti}\hlopt{$}\hlstd{Population),}\hlstr{"mean"}\hlstd{))\{}
  ...
  \hlkwa{if}\hlstd{(}\hlopt{!}\hlstd{cop} \hlopt{%in%} \hlkwd{c}\hlstd{(}\hlkwd{levels}\hlstd{(roti}\hlopt{$}\hlstd{Copper),}\hlstr{"mean"}\hlstd{))\{}
    \hlkwd{warning}\hlstd{(}\hlstr{"Something went wrong, give the instructors 
            a cookie and they may help you out:
