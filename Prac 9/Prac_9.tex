...
\end{kframe}
\end{knitrout}
This means that after $40$ years, $41\% $ of the total population will have originated from the two individuals that were initially in age class $1$, $29\% $ from the individual in age class $2$ and the remaining $30\% $ of the total population at timestep $40$ from the individual that was in age class $3$ in the initial population. These numbers do not correspond to the left eigenvector, yet they do relate to the left eigenvalue.
...
\hlstd{sens} \hlkwb{<-} \hlkwa{function}\hlstd{(}\hlkwc{A}\hlstd{)\{}
  ...
  \hlstd{outcome} \hlkwb{<-} \hlstd{v} \hlopt{%*%} \hlkwd{t}\hlstd{(w)} \hlopt{/} \hlkwd{as.numeric}\hlstd{(}\hlkwd{t}\hlstd{(w)}\hlopt{%*%}\hlstd{v)}
...
\end{knitrout}
\end{mdframed}
This is a very good start! We now have a function that easily provides us with the sensitivity values for any matrix. All we have to do now to get the sensitivity of some matrix \texttt{P} is to type \texttt{sens(P)}. 
