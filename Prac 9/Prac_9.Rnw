...
<<LAST1,include=FALSE,eval=FALSE>>=
### Interactions
temp2  <- Rates[,c('Copper','Layer','SaM','MM','SjM','RM')]
tempi <- aggregate(.~Copper+Layer,temp2,mean)
interaction <- list(9)
for(i in 1:9){
  ...
  interaction[[i]] <- LTRE(mat,MatMean) - layer[[as.character(tempi$Layer[i])]] - copper[[as.character(tempi$Copper[i])]]
}
