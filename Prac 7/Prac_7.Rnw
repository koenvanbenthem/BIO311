...
options(width=60)#,tikzDefaultEngine= 'xetex')
@
...
<<t2,echo=T,eval=F>>=
Nt <- N0*(lambda)^t
plot(t, Nt, xlab="time", ylab="N", xaxt = "n")
axis(1, at=0:4, labels=2009:2013)
@
...
<<t8,echo=T,eval=F>>=
N0 <- #insert initial value here
r <-  #insert instantaneous rate of increase here 
t <- 0:10
Nt <- ...*exp(...)
@
...
<<t9,echo=T,eval=F>>=
plot(t, Nt, xlab="time", ylab="N", type="l")
@
...
<<t19,include=F>>=
#setwd("C://Users//cornioley_t//Dropbox//Psch//course material pop ecol 2014//Prac 7")  
@
...
<<t020,echo=T,eval=T, results='hide'>>=
Bea <- read.csv("beaver_pop.csv", sep=";", header=T)  
head(Bea)
str(Bea)
@
...
<<t21,echo=T,eval=F>>=
options(scipen=10)
plot(Bea$Year, Bea$Population, xlab="year", ylab="N") 
@
...
<<t22,echo=T,eval=F>>=
plot(Bea$Year, log(Bea$Population), xlab="year", ylab="ln(N)") 
@
...
<<t23,echo=T,eval=T, results='hide'>>=
reg1 <- lm(log(Bea$Population) ~ Bea$Year)
reg1 # Use this to take a look at reg1
@
...
<<t24,echo=T,eval=F>>=
Bea$sim <- ... * exp(... * (Bea$Year))
@

<<t24b,echo=F,eval=T, results='hide'>>=
Bea$sim <- exp(reg1$coef[[1]]) * exp(reg1$coef[[2]] * (Bea$Year))
@
And plot it on the same graph as the original population. 
<<t25,echo=T,eval=T>>=
plot(Bea$Year, Bea$Population, xlab="year", ylab="N") 
lines(Bea$Year,  Bea$sim)
@
...
\end{Exercise}
\begin{Answer}[ref=fd]
$N_t=e^{\Sexpr{round(reg1$coef[[1]],3)}}\cdot e^{\Sexpr{round(reg1$coef[[2]],3)}t}$

<<t025,echo=T,eval=T>>=
lambda1<-exp(reg1$coef[[2]])
lambda1
@
$\lambda=\Sexpr{round(lambda1,3)}$

$N_t=e^{\Sexpr{round(reg1$coef[[1]],3)}}\cdot \Sexpr{round(lambda1,3)}^t$
\end{Answer}
...
lambda <- (Nt / N0) ^ (1 / (length(Bea$Population) - 1))
lambda
@
...
$N_t=\Sexpr{Bea$Population[1]}\cdot \Sexpr{round(lambda, 3)}^t$


$\lambda=\Sexpr{round(lambda, 3)}$
<<t027,echo=T,eval=T>>=
r1<-log(lambda)
r1
@

$N_t=\Sexpr{Bea$Population[1]}\cdot e^{\Sexpr{round(r1, 3)}t}$
...
plot(Bea$Year, Bea$Population, xlab="year", ylab="N") 
options(scipen=10)
lines(Bea$Year,  Bea$sim) #line from the linear regression
lines(Bea$Year,Bea$Population[1]*
        lambda^(0:(length(Bea$Population)-1))
      ,col='red') #line from the average growth rate
@
...
<<t30,echo=T,eval=T,out.width='0.65\\textwidth',fig.align='center'>>=
Beasub<-Bea[1:60,]

#From the fit
reg2 <- lm(log(Beasub$Population)~Beasub$Year)
Beasub$sim<-exp( reg2$coef[[1]])*exp( reg2$coef[[2]]*(Beasub$Year))

#From the average growth rate
N02<-Beasub$Population[1]
Nt2<-Beasub$Population[length(Beasub$Population)]

lambda2<-(Nt2/N02)^(1/(length(Beasub$Population)-1))
lambda2

#The plots 
plot(Bea$Year, Bea$Population, xlab="year", ylab="$N$") 
options(scipen=10)
lines(Bea$Year,  Bea$sim, col='black') 
lines(Bea$Year,Bea$Population[1]*lambda^(
  0:(length(Bea$Population)-1)),col='red')

lines(Beasub$Year,  Beasub$sim, col='black', lty=2)
lines(Beasub$Year,Beasub$Population[1]*lambda2^(
  0:(length(Beasub$Population)-1)),col='red', lty=2) 
legend("topleft",
       c("fit full ","fit subset", "avg full", "avg subset"), 
       bty = "n",
lty=c(1,2,1,2),
...
<<ksetpar,echo=FALSE,include=FALSE>>=
rmax<-0.7
K<-450
@
\section{Logistic Growth}
\subsection{Continuous Logistic Model}
So far we have assumed that the rabbits have a constant growth rate (where we found that $r=\Sexpr{rmax}$), regardless of the population size. In reality however, this growth rate is the maximum possible growth rate. In larger populations the actual growth rate is smaller than this maximal value (due to competition for food). Below we show how the growth rate depends on the population size: 

<<k1,eval=T,echo=F,out.width="0.7\\textwidth",fig.align="center">>=
N<-0:(K+50)
r<-rmax-(rmax/K)*N
par(mar=c(6,6,2,2))
plot(N,r,type="l",xlab="$N$",ylab="$r$",xlim=c(0,max(N)),
     ylim=c(0,rmax+0.1),yaxs="i",xaxs="i",cex.lab=1.5)
abline(v=0,col='gray')
abline(h=0,col='gray')
@
...
<<k8,eval=F>>=
K <-     # Input your value of K here
rmax <-  # Input the value of rmax here
N0 <- 300
N <- K / (... * exp(... * time))
@
<<k08,include=FALSE>>=
N0<-300
N<-K/(1+((K-N0)/N0)*exp(-rmax*time))
@
Now we plot the behaviour of $N$ against time:
<<k9,echo=c(2),eval=T>>=
par(mar=c(6,6,2,2))
plot(time, N, ylab="N", xlab="t", type="l", ylim=c(200,500))
@
...
<<k12,echo=FALSE,results='asis'>>=
N2 <- N1*exp(rd*(1-N1/K))
N3 <- N2*exp(rd*(1-N2/K))
N4 <- N3*exp(rd*(1-N3/K))
cat("$N_2 = ",round(N2,1),"$\\\\$N_3 = ",round(N3,1),"$\\\\$N_4 = ",round(N4,1),"$")
@
...
<<k18,echo=c(1),out.width="0.65\\textwidth",fig.align='center'>>=
N[100]
par(mar=c(6,6,2,2))
plot(1:100,N,xlab="t",ylab="N",type="l")
@
...
<<k19,out.width="0.5\\textwidth",echo=c(1,3),fig.align='center'>>=
runif(7)
par(mar=c(6,6,2,2))
hist(runif(10000),n=20,main="",
     xlab="value of $10.000$ random numbers", cex.lab=1.5)
@
...
for(rep in 1:30){
  ...
  lines(1:100,N,col=linecols[rep])
}
@
...
<<k23,eval=FALSE>>=
# First we create an empty plot that will later
# contain the different lines. We set the range
# of the axes (xlim,ylim) and by setting type=
# "n", we specify that no data should be
# plotted yet.

plot(0,0,xlim=c(0,100),ylim=c(300,450),
     type="n",xlab="time",ylab="N")

# We define 30 colors that we will use to plot
linecols=rainbow(30)

# Now we start the loop, in this example,
# we created 30 realisations of the stochastic
# model:
for(rep in 1:30){
  ...
  # Here you have to put your code for creating
  # 1 realisation of the stochastic growth model
  # (this includes a loop)
  
  # Finally we add a line with this new realisation to
  # the current plot
  lines(1:100,N,col=linecols[rep])
}
@
...
a <- runif(1)
cat(a)
if (a < 0.6) {b<-"survives"} else {b<-"dies"}
@
If this random number ($a$) is larger than $0.6$, the individual dies, if it is smaller than $0.6$, it survives. Because every number between $0$ and $1$ is equally likely to be drawn, this is the same thing as having a survival chance of $0.6$. In this case the individual thus \Sexpr{b}.
...
<<k25p3,eval=FALSE>>=
Np <- 1000
outcome <- ... # Create empty vector
for(i in 1:...){
  outcome[i] <- ...
}
hist(outcome, xlim=c(0,1))
@

<<k25p5,include=FALSE,eval=FALSE,echo=FALSE>>=
Np <- 1000
outcome_10 <- rep(0,Np)
